...
from nets.DFRnet import DFR, get_train_model
import numpy as np
from nets.DFRnet_training import get_lr_scheduler
...
from utils.dataloader import DFRnetDatasets
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------#
    train_annotation_path = r'.\data_path\train.txt'
    # val_annotation_path     = r'.\data_path\val.txt'
    ...
    model_body, prediction_model = DFR([input_shape[0], input_shape[1], 3], num_classes=num_classes,
                                       backbone=backbone, mode='train')
    if model_path != '':
        # ------------------------------------------------------#
        #   载入预训练权重
        # ------------------------------------------------------#
    ...
    if True:
        ...
        model.compile(optimizer=optimizer, loss={'DFR_loss': lambda y_true, y_pred: y_pred})
        # ---------------------------------------#
        #   获得学习率下降的公式
        # ---------------------------------------#
        lr_scheduler_func = get_lr_scheduler(lr_decay_type, Init_lr_fit, Min_lr_fit, UnFreeze_Epoch)

        epoch_step = num_train // 4
        # epoch_step_val      = num_val // 4

        # if epoch_step == 0 or epoch_step_val == 0:
        #     raise ValueError('数据集过小，无法进行训练，请扩充数据集。')

        train_dataloader = DFRnetDatasets(train_lines, input_shape, batch_size, num_classes, train=True)
        # val_dataloader      = DFRnetDatasets(val_lines, input_shape, batch_size, num_classes, train = False)
        ...
        if ngpus_per_node > 1:
            ...
            checkpoint = ModelCheckpoint(
                ...
                monitor='val_loss', save_weights_only=False, save_best_only=False, period=save_period)
